    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Kelola stok dan ketersediaan obat secara langsung.</p>
            </div>
            <button @click="showModal = true; isEdit = false; formAction = '{{ route('medis.obat.store') }}'; formData = { nama_obat: '', nama_batch: '', satuan: '', jenis_obat: 'tablet', expired_date: '', stok_saat_ini: '', reorder_level: '' };" class="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-md text-sm whitespace-nowrap">
                Tambah Obat
            </button>
        </div>
        ...
        <div class="p-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50/50">
            <form method="GET" class="flex items-center gap-2 w-full md:w-1/2">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama obat atau batch..." class="w-full px-4 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none text-sm transition-colors" {{ request('search') ? 'autofocus' : '' }} onfocus="var val = this.value; this.value = ''; this.value = val;" oninput="clearTimeout(this.delay); this.delay = setTimeout(() => this.form.submit(), 500)">
            </form>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 font-bold {{ $o->stok_saat_ini <= $o->reorder_level ? 'text-red-600' : 'text-emerald-700' }}">{{ $o->stok_saat_ini }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ \Carbon\Carbon::parse($o->expired_date)->format('Y-m-d') }}</td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <button @click="showModal = true; isEdit = true; formAction = '/medis/obat/{{ $o->id_obat }}'; formData = { nama_obat: '{{ addslashes($o->nama_obat) }}', nama_batch: '{{ addslashes($o->nama_batch) }}', satuan: '{{ addslashes($o->satuan) }}', jenis_obat: '{{ $o->jenis_obat }}', expired_date: '{{ \Carbon\Carbon::parse($o->expired_date)->format('Y-m-d') }}', stok_saat_ini: '{{ $o->stok_saat_ini }}', reorder_level: '{{ $o->reorder_level }}' };" class="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors">Edit</button>
                            <form action="{{ route('medis.obat.destroy', $o->id_obat) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus obat ini?');">
                                @csrf @method('DELETE')
        ...
        @if($obat->hasPages())
            <div class="p-4 border-t border-gray-100">{{ $obat->links() }}</div>
        @endif
    </div>
                    ...
                    <form :action="formAction" method="POST" class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                            <div>
                                ...
                                <input type="text" name="nama_obat" x-model="formData.nama_obat" placeholder="Masukkan nama obat..." required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Batch</label>
                                <input type="text" name="nama_batch" x-model="formData.nama_batch" placeholder="Contoh: BATCH-001" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Satuan</label>
                                <input type="text" name="satuan" x-model="formData.satuan" placeholder="Contoh: Pcs, Strip, Botol" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Jenis Obat</label>
                                <select name="jenis_obat" x-model="formData.jenis_obat" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none transition-colors bg-white">
                            ...
                            <div>
                                ...
                                <input type="date" name="expired_date" x-model="formData.expired_date" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                            </div>
                            <div class="grid grid-cols-2 gap-5">
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">Stok Saat Ini</label>
                                    <input type="number" name="stok_saat_ini" x-model="formData.stok_saat_ini" placeholder="0" required min="0" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">Batas Minimum</label>
                                    <input type="number" name="reorder_level" x-model="formData.reorder_level" placeholder="0" required min="0" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-end gap-3">
                            ...
                            <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-colors shadow-md">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
