@extends('layouts.app')
@section('title', 'Departemen - Klinik JAI')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Kelola data master departemen pegawai.</p>
            </div>
            <button @click="showModal = true; isEdit = false; formAction = '{{ route('admin.departments.store') }}'; formData = { name: '' };" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-md text-sm whitespace-nowrap">
                Tambah Departemen
            </button>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50">
                        <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-100">Nama Departemen</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <button @click="showModal = true; isEdit = true; formAction = '/admin/departments/{{ $d->id }}'; formData = { name: '{{ addslashes($d->name) }}' };" class="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors">Edit</button>
                            <form id="delete-form-{{ $d->id }}" action="{{ route('admin.departments.destroy', $d->id) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                            <button type="button" @click="confirmDelete('delete-form-{{ $d->id }}')" class="text-red-600 hover:text-red-800 font-medium text-sm transition-colors">Hapus</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="2" class="px-6 py-8 text-center text-gray-500 font-medium">Belum ada data departemen.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            <div x-show="showModal" class="fixed inset-0 z-[110] flex items-center justify-center p-4" x-transition style="display: none;">
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden" @click.stop>
                    <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-gray-800" x-text="isEdit ? 'Edit Departemen' : 'Tambah Departemen'"></h3>
                    ...
                    <form :action="formAction" method="POST" class="p-6">
                        ...
                        <div class="mb-6">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Departemen</label>
                            <input type="text" name="name" x-model="formData.name" required placeholder="Masukkan Nama Departemen..." class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none transition-colors">
                        </div>
