...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">Laporan Penyerahan Kwitansi</h2>
    </div>

    <form action="{{ route('admin.laporan.kwitansi') }}" method="GET" class="flex flex-wrap gap-4 mb-6 items-end bg-white p-4 rounded shadow">
        <div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.laporan.kwitansi.export', request()->all()) }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow transition">Export Excel</a>
        </div>
    </form>
        ...
        <table class="w-full whitespace-no-wrap table-auto">
            <thead>
                <tr class="text-left font-bold text-gray-700 bg-gray-100 border-b">
                    ...
                    <th class="px-6 py-4">Tanggal Lunas</th>
                </tr>
            </thead>
            <tbody>
                @forelse($data as $item)
                <tr class="border-b hover:bg-gray-50">
                    <td class="px-6 py-4">{{ $item->tanggal_terbit }}</td>
                    <td class="px-6 py-4">{{ $item->id_rekam_medis }}</td>
                    <td class="px-6 py-4">{{ $item->rekamMedis->pegawai->name ?? '-' }}</td>
                    <td class="px-6 py-4">Rp {{ number_format($item->total_biaya, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 capitalize">{{ str_replace('_', ' ', $item->status) }}</td>
                    <td class="px-6 py-4">{{ $item->tanggal_lunas ?? '-' }}</td>
                </tr>
                @empty
                <tr>
