@extends('layouts.app')
@section('title', 'Penyakit - Klinik JAI')
@section('header', 'Master Penyakit')

@section('content')
<div x-data="{ showModal: false, isEdit: false, formAction: '', formData: { nama_penyakit: '' } }">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Kelola data master diagnosis penyakit.</p>
            </div>
            <button @click="showModal = true; isEdit = false; formAction = '{{ route('admin.penyakit.store') }}'; formData = { nama_penyakit: '' };" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-md text-sm whitespace-nowrap">
                Tambah Penyakit
            </button>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <button @click="showModal = true; isEdit = true; formAction = '/admin/penyakit/{{ $p->id_penyakit }}'; formData = { nama_penyakit: '{{ addslashes($p->nama_penyakit) }}' };" class="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors">Edit</button>
                            <form id="delete-form-{{ $p->id_penyakit }}" action="{{ route('admin.penyakit.destroy', $p->id_penyakit) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                            <button type="button" @click="confirmDelete('delete-form-{{ $p->id_penyakit }}')" class="text-red-600 hover:text-red-800 font-medium text-sm transition-colors">Hapus</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="2" class="px-6 py-8 text-center text-gray-500 font-medium">Belum ada data penyakit.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden" @click.stop>
                    ...
                    <form :action="formAction" method="POST" class="p-6">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="nama_penyakit" x-model="formData.nama_penyakit" required placeholder="Masukkan Nama Penyakit..." class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none transition-colors">
                        </div>
